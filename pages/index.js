...
const DUA_AUDIO_URLS = [
  ...
  'https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/3.mp3',
  'https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/4.mp3',
  'https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/5.mp3',
  'https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/6.mp3',
];

// Quran reciters -- VERIFIED slugs from quranicaudio.com, surah numbers 3-digit zero-padded
const QURAN_RECITERS = [
  { slug: 'yasser_ad-dussary',              name: 'Yasser Al-Dosari',           style: 'Murattal' },
  { slug: 'mishaari_raashid_al_3afaasee',   name: 'Mishary Rashid Alafasy',     style: 'Murattal' },
  { slug: 'abu_bakr_ash-shaatree',          name: 'Abu Bakr al-Shatri',         style: 'Murattal' },
  { slug: 'abdul_basit_murattal',           name: 'AbdulBaset AbdulSamad',      style: 'Murattal' },
  { slug: 'abdulbaset_mujawwad',            name: 'AbdulBaset AbdulSamad',      style: 'Mujawwad' },
  { slug: 'maher_256',                      name: 'Maher Al-Muaiqly',           style: 'Murattal' },
  { slug: 'mahmood_khaleel_al-husaree',     name: 'Mahmoud Al-Husary',          style: 'Murattal' },
  { slug: 'rifai',                          name: 'Hani ar-Rifai',              style: 'Murattal' },
  { slug: 'abdurrahmaan_as-sudays',         name: 'Abdur-Rahman as-Sudais',     style: 'Murattal' },
  { slug: 'sa3ood_al-shuraym',              name: "Sa'ud ash-Shuraym",          style: 'Murattal' },
  { slug: 'khaalid_al-qahtaanee',           name: 'Khalid Al-Qahtani',          style: 'Murattal' },
  { slug: 'ahmed_ibn_3ali_al-3ajamy',       name: 'Ahmed ibn Ali al-Ajamy',     style: 'Murattal' },
  { slug: 'muhammad_siddeeq_al-minshaawee', name: 'Muhammad Siddiq Al-Minshawi',style: 'Murattal' },
];
...
export default function Home() {
  ...
  const [deckButtons,  setDeckButtons]  = useState([
    { id:4, icon:'⏯',  label:'Play',       action:'/media/play',      glow:'#a855f7' },
    { id:5, icon:'⏭',  label:'Next',       action:'/media/next',      glow:'#6366f1' },
    { id:6, icon:'⏮',  label:'Prev',       action:'/media/prev',      glow:'#6366f1' },
    { id:7, icon:'🌐', label:'Browser',    action:'/app/browser',     glow:'#3b82f6' },
    { id:8, icon:'💻', label:'Terminal',   action:'/app/terminal',    glow:'#22c55e' },
    { id:9, icon:'📷', label:'Screenshot', action:'/app/screenshot',  glow:'#f59e0b' },
  ]);
  ...
  const [messages,     setMessages]     = useState([
    { role:'assistant', content:"As-salamu alaykum Muntasir! 🌙 I'm Furqan, your Islamic assistant. Ask me anything about Islam, prayer, or the Quran." },
  ]);
  ...
  // ── Weather fetch ──
  useEffect(() => {
    const cached = cacheGet('weather_dhaka');
    if (cached) { setWeather(cached); return; }
    fetch('https://api.open-meteo.com/v1/forecast?latitude=23.8103&longitude=90.4125&current=temperature_2m,weathercode,windspeed_10m,relative_humidity_2m&timezone=Asia%2FDhaka')
      .then(r => r.json())
      .then(data => {
        if (data.current) {
          const w = {
  ...
  // ── Standby timer ──
  useEffect(() => {
    if (!standbyEnabled) { setStandby(false); return; }
    const resetActivity = () => { lastActivityRef.current = Date.now(); setStandby(false); };
    const events = ['touchstart','mousedown','keydown','mousemove'];
    events.forEach(e => window.addEventListener(e, resetActivity, { passive: true }));
    const check = setInterval(() => {
      if (Date.now() - lastActivityRef.current > 5 * 60 * 1000) setStandby(true);
    }, 10000);
    return () => { events.forEach(e => window.removeEventListener(e, resetActivity)); clearInterval(check); };
  }, [standbyEnabled]);
  ...
  const playFromVerse = (surahNum, verseNum) => {
    // Use full surah from quranicaudio.com, seeking to verse not supported per-verse
    // Instead just play full surah starting from beginning
    playRecitation(surahNum);
  };
  ...
  const getDuaUrl = (surahNum, reciterKey) =>
    `https://cdn.islamic.network/quran/audio-surah/128/${reciterKey}/${surahNum}.mp3`; // islamic.network doesn't need padding
  ...
  const playRecitation = (surahNum) => {
    ...
    const url = `https://download.quranicaudio.com/quran/${selQuranReciter.slug}/${String(surahNum).padStart(3,'0')}.mp3`;
    ...
    <>
      ...
      <div className="app-rotate">
        ...
        {authed && standby && standbyEnabled && (
          <div className="standby-screen" onClick={() => { setStandby(false); lastActivityRef.current = Date.now(); }}>
            <div className="standby-time">{formatTime(now)}</div>
            <div className="standby-date">{formatDate(now)}</div>
            {hijriLabel && <div className="standby-hijri">{hijriLabel}</div>}
            <div className="standby-tap">Tap to wake</div>
          </div>
        )}
        ...
        {!authed && (
          <div style={{
            ...
            <div style={{textAlign:'center'}}>
              <div style={{fontFamily:"'Amiri', serif", fontSize:'2.4rem', color:'#cc2244', marginBottom:'6px', lineHeight:1}}>بِسْمِ اللَّهِ</div>
              <div style={{fontSize:'1.2rem', fontWeight:800, color:'#fff', letterSpacing:'-0.02em'}}>Islamic Dashboard</div>
              <div style={{fontSize:'0.68rem', color:'#555', marginTop:'4px', letterSpacing:'0.06em', textTransform:'uppercase'}}>Private Access</div>
            </div>
            ...
            <div style={{
              ...
              <input
                style={{
                  ...
                  borderRadius:'12px', padding:'11px 14px', color:'#fff', fontSize:'0.9rem',
              ...
              <button
                ...
                style={{
                  background: loginLoading ? 'rgba(204,34,68,0.5)' : '#cc2244',
                  border:'none', borderRadius:'12px', padding:'12px',
                  color:'#fff', fontSize:'0.9rem', fontWeight:700,
                  cursor: loginLoading ? 'not-allowed' : 'pointer',
                  fontFamily:'Inter, sans-serif', transition:'all 0.2s',
                  boxShadow: loginLoading ? 'none' : '0 4px 20px rgba(204,34,68,0.4)',
                }}
              >
            ...
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:'0.68rem', color:'#444', marginBottom:'8px'}}>Not Muntasir?</div>
              <button
                onClick={() => { setIsPublic(true); setAuthed(true); }}
                style={{
        ...
        <div className="shell">
          ...
          <div className="topbar">
            <div className="topbar-left">
              <div className="topbar-greeting">As-salamu alaykum, {isPublic ? 'Guest' : displayName}</div>
            ...
            {weather && (
              <div style={{
                ...
                fontSize:'0.72rem', color: T.dim,
              }}>
                <span style={{fontSize:'1rem'}}>{getWeatherIcon(weather.code)}</span>
                <span style={{fontWeight:700, color:T.text}}>{weather.temp}°C</span>
                <span style={{color:T.dim}}>{getWeatherLabel(weather.code)}</span>
              </div>
            )}
            <div className="topbar-right">
              <button className="icon-btn" onClick={() => {
                const next = !standbyEnabled;
                setStandbyEnabled(next);
                try { localStorage.setItem('standby_enabled', String(next)); } catch {}
              }} title={standbyEnabled ? 'Standby On' : 'Standby Off'}
                style={{fontSize:'0.7rem', fontWeight:700, color: standbyEnabled ? T.green : T.dim}}>
                {standbyEnabled ? '🌙' : '💤'}
              </button>
              <button className="icon-btn" onClick={() => setTheme(t => {
              ...
              {isPublic ? (
                <button className="icon-btn" onClick={() => {
                  if (publicAiEnabled) { setActiveTab('home'); setChatOpen(o => !o); }
                  else setAiDisclaimer(true);
                }} title="Chat with Furqan (AI)">
                  💬
                </button>
              ) : (
                <button className="icon-btn" onClick={() => { setActiveTab('home'); setChatOpen(o => !o); }} title="Chat with Furqan">
                  💬
                </button>
              )}
            </div>
          ...
          <div className="content">
              ...
              <div className={`home-grid fadein${chatOpen ? '' : ' chat-closed'}`}>
                ...
                {chatOpen && (!isPublic || publicAiEnabled) && <div className="home-right">
                  <div className="chat-header">
                    ...
                    <div>
                      <div className="chat-title">Furqan</div>
                  ...
                  <div className="msgs">
                    ...
                    {chatLoading && <div className="bubble assistant loading pulse">Furqan is thinking...</div>}
                    <div ref={chatEndRef}/>
                  </div>
                  <div className="inputrow">
                    <textarea
                      className="cinput"
                      rows={1}
                      placeholder="Ask Furqan anything..."
                      value={input}
                    ...
                    <div className="verses-header">
                      ...
                      <div style={{display:'flex', alignItems:'center', gap:'6px', flexShrink:0}}>
                        <select
                          ...
                          style={{background: T.glass, backdropFilter: T.glassBlur, WebkitBackdropFilter: T.glassBlur, border: `1px solid ${T.glassBorder}`, borderRadius:'8px', color:T.dim, fontSize:'0.62rem', padding:'4px 6px', fontFamily:'Inter, sans-serif', cursor:'pointer', maxWidth:'120px'}}
                        >
            ...
            {activeTab === 'duas' && (
              <div className="duas-shell fadein">
                ...
                <div style={{background: T.glass, backdropFilter: T.glassBlur, WebkitBackdropFilter: T.glassBlur, border: `1px solid ${T.glassBorder}`, borderRadius:'14px', padding:'10px 14px', flexShrink:0}}>
              ...
              <div className="fadein" style={{
                ...
                <div style={{
                  ...
                  <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                    <div style={{width:'6px', height:'6px', borderRadius:'50%', background: pcStatus ? '#22c55e' : '#444', boxShadow: pcStatus ? '0 0 6px #22c55e' : 'none'}}/>
                    <div style={{fontSize:'0.65rem', fontWeight:700, color:'#555', letterSpacing:'0.12em', textTransform:'uppercase'}}>Stream Deck</div>
                    {pcStatus && <div style={{fontSize:'0.62rem', color:'#444', marginLeft:'4px'}}>{pcStatus.nowPlaying || 'Nothing playing'}</div>}
                  </div>
                  <button onClick={fetchPcStatus} style={{background:'transparent', border:'1px solid #222', borderRadius:'6px', padding:'3px 8px', color:'#444', fontSize:'0.6rem', fontWeight:700, cursor:'pointer', fontFamily:'Inter, sans-serif', letterSpacing:'0.08em'}}>PING</button>
                </div>

                {/* Main deck body */}
                <div style={{flex:1, display:'flex', flexDirection:'column', padding:'12px 10px', gap:'12px', overflowY:'auto'}}>

                  {/* Volume knob row */}
                  <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:'24px', padding:'8px 0'}}>
                    {/* Vol Down */}
                    <button
                      onClick={() => deckAction({id:'vd', action:'/volume/down'})}
                      style={{
                        width:'44px', height:'44px', borderRadius:'50%',
                        background:'linear-gradient(145deg, #1e1e1e, #111)',
                        border:'2px solid #2a2a2a',
                        boxShadow:'0 4px 12px rgba(0,0,0,0.8), inset 0 1px 0 #333',
                    ...
                    <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:'6px'}}>
                      <div style={{
                        ...
                        boxShadow:'0 6px 20px rgba(0,0,0,0.9), inset 0 2px 0 #333, 0 0 0 1px #0a0a0a',
                        display:'flex', flexDirection:'column',
                        alignItems:'center', justifyContent:'center',
                        position:'relative', cursor:'pointer',
                      }}
                        onClick={() => deckAction({id:'vm', action:'/volume/mute'})}
                      >
                        {/* Knob indicator dot */}
                        <div style={{
                          ...
                          background: pcStatus?.muted ? '#cc2244' : '#22c55e',
                          boxShadow: pcStatus?.muted ? '0 0 6px #cc2244' : '0 0 6px #22c55e',
                        }}/>
                        <div style={{fontSize:'1.1rem', lineHeight:1}}>🔊</div>
                        <div style={{fontSize:'0.7rem', fontWeight:800, color:'#888', marginTop:'2px', fontVariantNumeric:'tabular-nums'}}>
                          {pcStatus ? `${pcStatus.volume}%` : '--'}
                        </div>
                      </div>
                      <div style={{fontSize:'0.55rem', color:'#333', letterSpacing:'0.12em', textTransform:'uppercase', fontWeight:700}}>Volume</div>
                    </div>

                    {/* Vol Up */}
                    <button
                      onClick={() => deckAction({id:'vu', action:'/volume/up'})}
                      style={{
                        width:'44px', height:'44px', borderRadius:'50%',
                        background:'linear-gradient(145deg, #1e1e1e, #111)',
                        border:'2px solid #2a2a2a',
                  ...
                  {/* Divider */}
                  <div style={{height:'1px', background:'linear-gradient(to right, transparent, #1e1e1e, transparent)'}}/>

                  {/* Button grid -- 3x2 */}
                  <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:'8px'}}>
                    {deckButtons.filter(b => !['vd','vu','vm'].includes(b.id)).map((btn, idx) => {
                      const fb = deckFeedback[btn.id];
                      const glowColor = btn.glow || '#3b82f6';
                      return (
                        <div
                          key={btn.id}
                          draggable
                          ...
                          onPointerUp={e => e.currentTarget.style.transform='scale(1)'}
                          style={{
                            background: fb === 'ok' ? '#0d1f0d' : fb === 'err' ? '#1f0d0d' : 'linear-gradient(160deg, #1a1a1a, #111)',
                            border: `1px solid ${fb === 'ok' ? '#22c55e44' : fb === 'err' ? '#ef444444' : '#222'}`,
                            borderRadius: '12px',
                            ...
                            minHeight: '64px',
                            boxShadow: fb === 'active'
                              ? `0 0 12px ${glowColor}66, inset 0 0 8px ${glowColor}22`
                              : '0 3px 8px rgba(0,0,0,0.6), inset 0 1px 0 #2a2a2a',
                        ...
                        >
                          ...
                          <div style={{
                            ...
                            height:'1px',
                            background: fb ? (fb==='err'?'#ef4444':fb==='ok'?'#22c55e':glowColor) : glowColor,
                            opacity: fb ? 1 : 0.3,
                            boxShadow: `0 0 6px ${glowColor}`,
                          }}/>
                          <span style={{fontSize:'1.4rem', lineHeight:1, filter: fb==='err'?'grayscale(1)':'none'}}>{btn.icon}</span>
                          <span style={{
                            fontSize:'0.58rem', fontWeight:700,
                            color: fb === 'ok' ? '#22c55e' : fb === 'err' ? '#ef4444' : '#444',
                  ...
                  <div style={{fontSize:'0.55rem', color:'#222', textAlign:'center', letterSpacing:'0.08em'}}>DRAG TO REORDER</div>
                </div>
              </div>
            )}
              ...
              <div className="settings-shell fadein">
                <div className="settings-row">
                  <div>
                    ...
                    <div className="settings-sub">{isDark ? 'Dark — Black with white accent' : 'Light — White with black accent'}</div>
                  </div>
                  <button className="toggle" onClick={() => setTheme(t => {
                    const next = t==='dark'?'light':'dark';
                    try { localStorage.setItem('theme', next); } catch {}
                    return next;
                  })} style={{background: isDark ? T.accent : T.accent}}/>
                </div>
                <div className="settings-row">
                  <div>
                    <div className="settings-label">Standby Clock</div>
                    <div className="settings-sub">Show minimal clock after 5 min inactivity</div>
                  </div>
                  <button className="toggle" onClick={() => {
                    const next = !standbyEnabled;
                    setStandbyEnabled(next);
                    try { localStorage.setItem('standby_enabled', String(next)); } catch {}
                  }} style={{background: standbyEnabled ? T.green : T.border}}/>
                </div>
                {!isPublic && (
                  <div className="settings-row" style={{flexDirection:'column', alignItems:'flex-start', gap:'6px'}}>
                    ...
                    <input
                      style={{
                        background: T.glass, backdropFilter: T.glassBlur,
                        border:`1px solid ${T.glassBorder}`, borderRadius:'10px',
                        padding:'8px 12px', color:T.text, fontSize:'0.82rem',
                        fontFamily:'Inter, sans-serif', outline:'none', width:'100%',
                      }}
                      value={displayName}
                      onChange={e => {
                        setDisplayName(e.target.value);
                        try { localStorage.setItem('display_name', e.target.value); } catch {}
                      }}
                ...
                {isPublic && (
                  <div className="settings-row">
                    <div>
                      ...
                      <div className="settings-sub">Furqan Islamic AI chat</div>
                    </div>
                    <button className="toggle" onClick={() => {
                      if (!publicAiEnabled) setAiDisclaimer(true);
                      else setPublicAiEnabled(false);
                    }} style={{background: publicAiEnabled ? T.green : T.border}}/>
                  </div>
                )}
                <div className="settings-row">
                  <div>
                ...
                <div className="settings-row">
                  ...
                  <button
                    ...
                    onClick={() => { setAuthed(false); setIsPublic(false); setLoginPass(''); setLoginUser(''); }}
                  >
                    Lock
                  </button>
                </div>
          ...
          <div className="bottomnav">
            {[
              ...
              ...(!isPublic ? [{ id:'deck', icon:'#', label:'Deck' }] : []),
        ...
        {aiDisclaimer && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{fontSize:'1rem', fontWeight:700, color:'#fff'}}>AI Assistant</div>
              <div style={{fontSize:'0.82rem', color:'rgba(255,255,255,0.6)', lineHeight:1.6}}>
                Use the AI, but verify the info. Islamic guidance should always be confirmed with a qualified scholar. Thank you!
              </div>
              <button onClick={() => { setPublicAiEnabled(true); setAiDisclaimer(false); setChatOpen(true); setActiveTab('home'); }}
                style={{background:'#cc2244', border:'none', borderRadius:'12px', padding:'11px', color:'#fff', fontSize:'0.88rem', fontWeight:700, cursor:'pointer', fontFamily:'Inter, sans-serif'}}>
                I Understand — Enable AI
              </button>
              <button onClick={() => setAiDisclaimer(false)}
                style={{background:'transparent', border:'1px solid rgba(255,255,255,0.15)', borderRadius:'12px', padding:'9px', color:'rgba(255,255,255,0.5)', fontSize:'0.82rem', cursor:'pointer', fontFamily:'Inter, sans-serif'}}>
                Cancel
              </button>
            </div>
          </div>
